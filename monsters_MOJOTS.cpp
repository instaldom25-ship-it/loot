...
void MonsterType::createLoot(Container* corpse)
{
	...
	if (g_config.getBoolean(ConfigManager::SHOW_MONSTER_LOOT)) {
		...
		if (owner) {
			...
			if (party) {
				// Wyœlij bia³y komunikat do leadera
				Player* leader = party->getLeader();
				if (leader) {
					leader->sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
				}

				// Wyœlij bia³y komunikat do wszystkich memberów party
				for (Player* member : party->getMembers()) {
					if (member) {
						member->sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
					}
				}
			}
			else {
				// Bez party – standardowo bia³y
				owner->sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
			}
		}
	}
