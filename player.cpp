...
//robienie run z bp i z hand
Item* Player::getFirstItemById(uint16_t itemId) const
{
	for (int32_t i = CONST_SLOT_FIRST; i <= CONST_SLOT_LAST; i++) {
		Item* item = inventory[i];
		if (!item) {
		...
		// item bezpoœrednio w slocie (np. armor, rêka, backpack w slocie)
		if (item->getID() == itemId) {
			return item;
		}

		// item w kontenerach (backpack i wszystkie nested plecaki)
